...
COPY --from=ghcr.io/jaysonsantos/bunderwar:cargo-deny-0.18.9 /usr/local/bin/cargo-deny /usr/local/bin/
...
COPY --from=ghcr.io/jaysonsantos/bunderwar:lima-2.0.2 /opt/lima/ /usr/local/

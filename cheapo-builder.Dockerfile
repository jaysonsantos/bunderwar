...
COPY --from=ghcr.io/jaysonsantos/bunderwar:cargo-outdated-0.19.0 /usr/local/bin/cargo-outdated /usr/local/bin/
COPY --from=ghcr.io/jaysonsantos/bunderwar:cargo-deny-0.20.2 /usr/local/bin/cargo-deny /usr/local/bin/
COPY --from=ghcr.io/jaysonsantos/bunderwar:cargo-udeps-0.1.60 /usr/local/bin/cargo-udeps /usr/local/bin/
COPY --from=ghcr.io/jaysonsantos/bunderwar:cargo-audit-0.22.2 /usr/local/bin/cargo-audit /usr/local/bin/
COPY --from=ghcr.io/jaysonsantos/bunderwar:sccache-0.16.0 /usr/local/bin/sccache /usr/local/bin/
COPY --from=ghcr.io/jaysonsantos/bunderwar:sqlx-cli-0.9.0 /usr/local/bin/ /usr/local/bin/
COPY --from=ghcr.io/jaysonsantos/bunderwar:cargo-zigbuild-0.23.0 /usr/local/bin/ /usr/local/bin/
COPY --from=ghcr.io/jaysonsantos/bunderwar:lima-2.1.4 /opt/lima/ /usr/local/

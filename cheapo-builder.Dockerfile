...
COPY --from=ghcr.io/jaysonsantos/bunderwar:cargo-deny-0.18.3 /usr/local/bin/cargo-deny /usr/local/bin/
COPY --from=ghcr.io/jaysonsantos/bunderwar:cargo-udeps-0.1.56 /usr/local/bin/cargo-udeps /usr/local/bin/
...
COPY --from=ghcr.io/jaysonsantos/bunderwar:lima-1.1.1 /opt/lima/ /usr/local/
